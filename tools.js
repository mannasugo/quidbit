...
class Tools {
  ...
  plotXY (Arg) {
    ...
    if (Arg[1] === `3M`) {
      ...
      let Z = Math.floor(new Date(Arg[3]).getMinutes()/3)*3*60000;
      ...
      for (let a = 0; a < (Arg[2]*3600000)/86400000; a++) {
        ...
        if (existsSync(`bin/data/spot/klines/json/180000/${Arg[0][0]}${Arg[0][1]}_-_${X_D - 60000*60*24*a}.json`) === true) {

          XY[0] = XY[0].concat(this.typen(readFileSync(`bin/data/spot/klines/json/180000/${Arg[0][0]}${Arg[0][1]}_-_${X_D - 60000*60*24*a}.json`, {encoding: `utf8`})));
        }
      ...
      for (let A = 0; A < Arg[2]; A++) {
        ...
        let D5 = X_Z - 180000*A, XYV = [];

        XY[0].forEach(Obj => { if (Obj[0] >= D5 && Obj[0] < D5 + 180000) { XYV.push(Obj) } });
        ...
        if (XYV.length > 0) {
          ...
          PlotXY.push([
            D5, 
            [XYV.sort((A, B) => {return A[0] - B[0]})[0][1][0], XYV.sort((A, B) => {return B[0] - A[0]})[0][1][1]], 
            [HL[0].sort((A, B) => {return B - A})[0], HL[1].sort((A, B) => {return A - B})[0]], 
            V]);
        }

        else {PlotXY.push([X_Z - 180000*A, [], [], 0])}
      }
    }
    ...
    if (Arg[1] === `5M`) {
      ...
      for (let a = 0; a < (Arg[2]*3600000)/86400000; a++) {
        ...
        if (existsSync(`bin/data/spot/klines/json/300000/${Arg[0][0]}${Arg[0][1]}_-_${X_D - 60000*60*24*a}.json`) === true) {

          XY[0] = XY[0].concat(this.typen(readFileSync(`bin/data/spot/klines/json/300000/${Arg[0][0]}${Arg[0][1]}_-_${X_D - 60000*60*24*a}.json`, {encoding: `utf8`})));
        }
      ...
      for (let A = 0; A < Arg[2]; A++) {
        ...
        let D5 = X_Z - 300000*A, XYV = [];

        XY[0].forEach(Obj => { if (Obj[0] >= D5 && Obj[0] < D5 + 300000) { XYV.push(Obj) } });
        ...
        else {PlotXY.push([X_Z - 300000*A, [], [], 0])}
      }
    }
    ...
    if (Arg[1] === `15M`) {
      ...
      for (let A = 0; A < Arg[2]; A++) {
        ...
        let D15 = X_Z - 900000*A, XYV = [];

        XY[0].forEach(Obj => { if (Obj[0] >= D15 && Obj[0] < D15 + 900000) { XYV.push(Obj) } });
    ...
    if (Arg[1] === `30M`) {
      ...
      for (let A = 0; A < Arg[2]; A++) {
        ...
        let D30 = X_Z - 1800000*A, XYV = [];

        XY[0].forEach(Obj => { if (Obj[0] >= D30 && Obj[0] < D30 + 1800000) { XYV.push(Obj) } });
    ...
    if (Arg[1] === `1H`) {
      ...
      for (let A = 0; A < Arg[2]; A++) {
        ...
        let D60 = X_Z - 3600000*A, XYV = [];

        XY[0].forEach(Obj => { if (Obj[0] >= D60 && Obj[0] < D60 + 3600000) { XYV.push(Obj) } });
    ...
    if (Arg[1] === `1D`) {
      ...
      XY[0].forEach(X => {XY[1][X[0]] = X});console.log(Arg[2])
      ...
      Constants.plot.forEach(Y => {
        ...
        HR(`https://api.coinbase.com/v2/prices/${key}/spot`, (flaw, State, value) => {
          ...
          if (!flaw && State.statusCode == 200 && this.typen(value) && this.typen(value).data && parseFloat(this.typen(value).data.amount) > 0) {
            ...
            if (!this.XY[key]) {this.XY[key] = {}}

            if (!this.XY[key][X_Z]) {this.XY[key][X_Z] = []}
      ...
      if (new Date().valueOf() > X_A + 60000)  {
        ...
        for (let plot in this.XY) {
          ...
          if (existsSync(`bin/data/spot/klines/json/60000/${plot.replace(`-`, ``)}_-_${DAY}.json`) === true) {
            ...
            XYV = this.typen(readFileSync(`bin/data/spot/klines/json/60000/${plot.replace(`-`, ``)}_-_${DAY}.json`, {encoding: `utf8`})).concat(XYV);
          }

          writeFileSync(`bin/data/spot/klines/json/60000/${plot.replace(`-`, ``)}_-_${DAY}.json`, this.coats(XYV));

          this.XY[plot] = {};
        }

        X_A = new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} ${new Date().getHours() }:${new Date().getMinutes()}`).valueOf();
      }
    }, 1000);
  ...
  plot24 () {
    ...
    Constants.plot.forEach(Plot => {
      ...
      if (existsSync(`bin/data/spot/klines/json/60000/${Plot[0][0]}${Plot[0][1]}_-_${DAY - 3600000*24}.json`) === true) {

        this.typen(readFileSync(`bin/data/spot/klines/json/60000/${Plot[0][0]}${Plot[0][1]}_-_${DAY - 3600000*24}.json`, {encoding: `utf8`})).forEach(XY => {
...
let Constants = {
  ...
  fiat: [
    ...
    [`CNY`, 5, hold, `chinese yuan`, `USD-CNY`],
    [`EUR`, 5, hold, `euro`, `EUR-USD`],
    [`GBP`, 5, hold, `sterling pound`, `GBP-USD`],
    [`INR`, 5, hold, `indian rupee`, `USD-INR`],
